  ...
  return async (req: Request): Promise<T | Response> => {
    try {
      ...
      const userData: AuthUser = {
        id: decoded.payload.sub as string, // 'sub' = user ID
        email: decoded.payload.email as string,
        name: decoded.payload.name as string,
        picture: decoded.payload.picture as string,
        given_name: decoded.payload.given_name as string,
        family_name: decoded.payload.family_name as string,
        email_verified: decoded.payload.email_verified as boolean,
        provider: decoded.payload.provider as string,
        exp: decoded.payload.exp ? Number(decoded.payload.exp) : undefined,
        cookieExpiration: decoded.payload.cookieExpiration
          ? Number(decoded.payload.cookieExpiration)
          : undefined, // Added for web cookie expiration tracking
      };

      // Call the handler with the authenticated user
      return await handler(req, userData);
    } catch (error) {
